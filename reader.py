...
# 读取图片数据
def load_data(data_dir,image_list,output_size=(512, 384), data_enhance=None):
...
#             print(image_list)
        ...
        if label is None:
            print("label无数据")
            sys.exit()
        naive_image,naive_label = resize_and_transpose(image,label,output_size)#BCHW

        image_output.append(naive_image)
        label_output.append(naive_label)

        # 使用数据增强等操作
        if data_enhance==1:#随机裁剪
